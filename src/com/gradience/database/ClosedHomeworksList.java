...
public class ClosedHomeworksList {
	...
	public ArrayList<Exercise> execute(String course_id) {
		...
		try {
			sttmnt = DBConnection.instance().conn.prepareCall("{call PAST_EXERCISE(?,?,?,?)}");
			...
			sttmnt.registerOutParameter("pastExerciseData", OracleTypes.CURSOR);
			sttmnt.execute();
			if (sttmnt.getString("MSG").equals("success")) {
				System.out.println(sttmnt.getString("TEXT"));
				ResultSet rs = (ResultSet) sttmnt.getObject("pastExerciseData");
				if(rs.isBeforeFirst()){
				while (rs.next()) {
